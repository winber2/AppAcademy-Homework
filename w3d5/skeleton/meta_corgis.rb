...
class MetaCorgiSnacks
  ...
  #   if [:treat, :kibble, :bone].include?(name)
  #     info = "get_#{name}_info".to_sym
  #     get_info = @snack_box.send info, @box_id
  #     taste = "get_#{name}_tastiness".to_sym
  #     get_taste = @snack_box.send taste, @box_id
  #     result = "#{name.capitalize}: #{get_info}: #{get_taste} "
  #     get_taste > 30 ? "* #{result}" : result
  #   else
  #     super
  #   end
  # end
  ...
  def self.define_snack(name)
    define_method(name) do
      ...
      result = "#{display_name}: #{info}: #{taste}"
